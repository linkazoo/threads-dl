...
Note that it works for both the timeline view and single post view, but it does not work directly when a post is embedded inside a quote card. In that case, you can open that post either in the timeline view or single post view to download its contents.
...
它會在每篇 Threads 貼文下方的「分享」按鈕旁邊，新增一個 __Download Media（下載媒體）__ 按鈕。點擊該按鈕後，系統會將所有媒體檔案下載至您的預設下載資料夾。下載的檔案名稱格式為：`username_YYMMDD_PostID-N.mp4/jpg`（其中 N 為檔案的順序編號）。
...
請注意，此擴充功能同時支援「動態時報（Timeline）檢視」與「單篇貼文檢視」，但無法直接下載內嵌在引用貼文（Quoted Post）中的媒體。若要下載該類媒體，您需要先點入該篇原始貼文，將其切換至動態時報或單篇貼文檢視中。
